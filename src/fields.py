...
class source_point:
    ...
    def __init__(self, sim, beam):  # constructor
        ...
        def F0(r, phi, z):
            return kn(0, r * alpha_b / b / gamma) - i0(r * alpha_b / b / gamma) * kn(
                0, x
            ) / i0(x)

        def dF0(r, phi, z):
            return -kn(1, r * alpha_b / b / gamma) - i1(r * alpha_b / b / gamma) * kn(
                0, x
            ) / i0(x)
...
class source_ring_bottom:
    ...
    def __init__(self, sim, beam):  # constructor
        ...
        def F0(r, phi, z):
            return kn(0, s) - i0(s) * kn(0, x) / i0(x)
...
class source_cylinder_top:
    ...
    def __init__(self, sim, beam):  # constructor
        ...
        self.Er = (
            lambda r, phi, z: Q * Z0 / (2 * np.pi * r) * np.exp(-1j * z * alpha_b / b)
        )
        self.Ephi = lambda r, phi, z: 0
        self.Ez = lambda r, phi, z: 0
        ...
        self.Hr = lambda r, phi, z: 0
        self.Hphi = (
            lambda r, phi, z: Q / (2 * np.pi * r) * np.exp(-1j * z * alpha_b / b)
        )
        self.Hz = lambda r, phi, z: 0
...
class Pipe:
    ...
    def __init__(self, sim, index_p, direction):  # constructor
        ...
        self.Ephi = lambda r, phi, z: 0
        self.Ez = (
            lambda r, phi, z: jn(0, r * alpha_p / b) / Tau_TM
        )  # * np.exp(-1j * direction * z * alpha_p_tilde / b)

        self.Hr = lambda r, phi, z: 0
        self.Hphi = (
            lambda r, phi, z: 1j * jn(1, r * alpha_p / b) / (Z0 * alpha_p * Tau_TM)
        )  # * np.exp(-1j * direction * z * alpha_p_tilde / b)
        self.Hz = lambda r, phi, z: 0
...
class cavity:
    ...
    def __init__(self, sim, index_r, index_s):  # constructor
        ...
        self.Ephi = lambda r, phi, z: 0
        self.Ez = (
            lambda r, phi, z: jn(0, r * alpha_r / d) / (V_rs) * np.cos(z * alpha_s / d)
        )
        self.Hr = lambda r, phi, z: 0
        self.Hphi = (
            lambda r, phi, z: alpha_rs
            * jn(1, r * alpha_r / d)
            / (alpha_r * V_rs)
            * np.cos(z * alpha_s / d)
        )
        ...
        #
        self.Q = (
            lambda0 / deltas * (alpha_rs / (2 * np.pi * (1 + epsilon_s(s) * d / L)))
        )
...
class cavity_project:
    ...
    def __init__(
        self, mode_num, mesh, zmatch, radius_CST=5, stepsize_CST=0.1, datadir="./CST"
    ):  # constructor
        ...
        if zmatch == 0:
            E_dati = np.loadtxt(
                datadir + "E_Mode_left {}.txt".format(mode_num), skiprows=2
            )
            H_dati = np.loadtxt(
                datadir + "H_Mode_left {}.txt".format(mode_num), skiprows=2
            )
        else:
            E_dati = np.loadtxt(
                datadir + "E_Mode_right {}.txt".format(mode_num), skiprows=2
            )
            H_dati = np.loadtxt(
                datadir + "H_Mode_right {}.txt".format(mode_num), skiprows=2
            )

        Ex = np.transpose(
            E_dati[:, 3].reshape(
                np.rint((2 * radius_CST + stepsize_CST) / stepsize_CST).astype(int),
                np.rint((2 * radius_CST + stepsize_CST) / stepsize_CST).astype(int),
            )
            / np.sqrt(2 / eps_0)
        )
        ...
        x = E_dati[
            : np.rint((2 * radius_CST + stepsize_CST) / stepsize_CST).astype(int), 0
        ]
        y = E_dati[
            :: np.rint((2 * radius_CST + stepsize_CST) / stepsize_CST).astype(int), 1
        ]
...
class cavity_project_on_axis:
    ...
    def __init__(self, mode_num, mesh, datadir="./CST"):  # constructor
        ...
        E_dati_on_axis = np.loadtxt(
            datadir + "Mode_on_axis {}.txt".format(mode_num), skiprows=2
        )
        ...
        interpolated_function = interp1d(
            Z_CST, Ez_CST, kind="linear", fill_value="extrapolate"
        )

        # Calcola i valori interpolati su mesh.Z
        self.Ez = interpolated_function(mesh.Z)

class cavity_project_on_side:

    def __init__(self, mode_num, mesh, datadir="./CST"):  # constructor
        from scipy.constants import epsilon_0 as eps_0
        from scipy.interpolate import interp1d

        E_dati_on_side = np.loadtxt(
            datadir + "Mode_on_side {}.txt".format(mode_num), skiprows=2
        )
        Ez_CST = E_dati_on_side[:, 7] / np.sqrt(2 / eps_0)
        Z_CST = E_dati_on_side[:, 2] * 1e-2
        self.Fz = np.zeros(mesh.Z.size)
        # self.Fz = np.zeros(self.Ez.size)
