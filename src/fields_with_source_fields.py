...
class Mesh_loss:
    ...
    def __init__(self, sim, Np=10):  # constructor
        ...
        # pipe_mesh = np.linspace(-1*sim.d, 1*sim.d, self.Np, endpoint=True)
...
class Mesh:
    ...
    def __init__(self, sim, Np=10):  # constructor
        ...
        self.Np = Np
        self.xi = np.linspace(-1 * sim.d, 1 * sim.d, self.Np, endpoint=True)
        # pipe_mesh = np.linspace(-1*sim.d, 1*sim.d, self.Np, endpoint=True)
        # beam_mesh = np.linspace(-2*sim.rb, 2*sim.rb, self.Np, endpoint=True)
        # self.xi = np.sort(np.unique(np.concatenate((beam_mesh, pipe_mesh))))
        self.yi = self.xi
...
class Simulation:
    ...
    def __init__(self, frequency=2e9, index_max_p=3, index_max_s=3):  # constructor
        ...
        self.ix_pairs_n = list(
            itertools.product(np.arange(self.index_max_p), np.arange(self.index_max_s))
        )
...
class Source:
    ...
    def __init__(self, sim, beam):  # constructor
        ...
        dF0 = lambda r, phi, z: -kn(1, r * alpha_b / b / gamma) - i1(
            r * alpha_b / b / gamma
        ) * kn(0, x) / i0(x)
...
class Cavity:
    ...
    def __init__(self, sim, index_p, index_s):  # constructor
        ...
        if sim.sigma != 0:
            deltas = np.sqrt(2 / omega0 / mu_0 / sim.sigma)
        else:
            deltas = np.inf
        self.deltas = deltas
        ...
        self.Ephi = lambda r, phi, z: 0
        self.Ez = (
            lambda r, phi, z: jn(0, r * alpha_p / d) / (V_ps) * np.cos(z * alpha_s / d)
        )

        self.Hr = lambda r, phi, z: 0
        self.Hphi = (
            lambda r, phi, z: alpha_ps
            * jn(1, r * alpha_p / d)
            / (alpha_p * V_ps)
            * np.cos(z * alpha_s / d)
        )
        ...
        self.Q = (
            lambda0 / deltas * (alpha_ps / (2 * np.pi * (1 + epsilon_s(s) * d / L)))
        )
