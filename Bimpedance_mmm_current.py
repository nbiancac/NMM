...
Zout = []
fout = []
sim = fields.simulation(index_max_p=P_max, index_max_r=R_max, index_max_s = S_max)
...
print(f"Tempo impiegato: {tempo_trascorso/60} minuti")
...
# np.savetxt('Arrays/Z_conv_imp_S'+str(S_max)+'_R'+str(R_max)+'_P'+str(P_max)+'_Np'+str(Np)+'.txt', Z_conv, fmt='%.16f%+.16fj')

plt.figure()
plt.plot(range1*1e-9, Z_conv.real)
# plt.legend(['real', 'imag'])
plt.title('Beam current: S = '+str(S_max)+' R = '+str(R_max)+', P = '+str(sim.index_max_p)+', Np = '+str(Np))
